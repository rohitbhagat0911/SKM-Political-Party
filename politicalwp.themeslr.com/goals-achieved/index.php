<?php
session_start();
require_once '../../server/goalsachived.php';
                            ...
                            <div class="entry-content">
                                <div data-vc-full-width="true" data-vc-full-width-init="false"
									...
									if (isset($_SESSION['isLoggedIn']) == true && ($_SESSION['userDegree'] == 0 || $_SESSION['userDegree'] == 1 )) {
											?>
				<a href="./?add=vision">Add Goals achieved</a> </br>
											<?php
																		if (isset($_GET['eid'])) {
																			if($gaDetail != FALSE && $gaDetail->num_rows > 0) {
																				while ($ga = $gaDetail->fetch_assoc()) {
																					if ($_GET['eid'] == $ga['id']) {
																						
																			?>
																				<form accept-charset="UTF-8" action="../../server/editgoalachieved.php" method="POST">
											
											<div class="form-group">
												<label for="description">Edit Achieved:</label>
												<textarea id="description" name="desc" class="form-control" rows="4" cols="50" maxlength="1000" required="required" placeholder="Edit Goal"><?php echo $ga['goals']; ?> </textarea>
											</div>
											<input type="hidden" name="id" value="<?php echo $ga['id']; ?>">
											<hr>
											<button type="submit" class="btn btn-primary" name="editga">Submit</button>
										</form>	
																			<?php
																					}
											...
											if ((isset($_GET['add']) && $_GET['add'] == 'vision')) {
												?>
												<form accept-charset="UTF-8" action="../../server/addgoalachieved.php" method="POST">
                
                <div class="form-group">
                    <label for="description">Enter goal achieved:</label>
                    <textarea id="description" name="desc" class="form-control" rows="4" cols="50" maxlength="1000" required="required" placeholder="Enter Goals"></textarea>
                </div>
                
                <hr>
                <button type="submit" class="btn btn-primary" name="addgoal">Submit</button>
            </form>
											...
											?>
                                                <div class="title-subtile-holder">
                                                    <h1 class="section-title dark_title">Goals Achieved
                                                    </h1>
                                                    </br>
                                                    <p class="section-title"><i>The following are the goals that we: </i></p>
													</br>
													<ul>
                                                    <?php

                                                        if($gaDetails != FALSE && $gaDetails->num_rows > 0) {
                                                            while ($gas = $gaDetails->fetch_assoc()) {
									if (isset($_SESSION['isLoggedIn']) == true && ($_SESSION['userDegree'] == 0 || $_SESSION['userDegree'] == 1 )) {

                                                            ?>
                                                        <a href="./?eid=<?php echo $gas['id']; ?>">Edit Goals achieved</a> </br>
                                                        <a href="../../server/deletegoalachieved.php?did=<?php echo $gas['id']; ?>">Delete Goals achieved</a> </br>
