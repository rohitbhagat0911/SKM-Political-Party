...
if (isset($_POST['enews'])) {
    ...
    if(!empty($_FILES["image"]["name"]) || !empty($_FILES["image2"]["name"]) || !empty($_FILES["image3"]["name"]) || !empty($_FILES["video"]["name"]))  { 
        ...
        if(in_array($fileType, $allowTypes)){ 
            ...
            $sql = "UPDATE eventform SET `image` = '$imgContent'";
            $db->query($sql);
        ...
        if (in_array($image2Type, $allowTypes)) {
            ...
            $sql = "UPDATE eventform SET `image2` = '$img2Content'";
            $db->query($sql);
        ...
        if (in_array($image3Type, $allowTypes)) {
            ...
            $sql = "UPDATE eventform SET `image3` = '$img3Content'";
            $db->query($sql);
        ...
        $sql = "UPDATE eventform SET `video` = '$vidContent'";
        $db->query($sql);
        ...
        $sql = "UPDATE eventform SET `title`='$title', `description`='$desc' WHERE `id`='$id'";
        $db->query($sql);
    ...
    if ($vidbadres == '') {
        header("Location: ../politicalwp.themeslr.com/event-form/index.php?res=newsupdated");
ob_end_flush();
    } else if($vidbadres == '1') {
        header("Location: ../politicalwp.themeslr.com/event-form/index.php?res=newsupdated&vb=1");
ob_end_flush();
    }
